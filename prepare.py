...
def main() -> int:
    ...
    # Existing script is only needed so the canonical loader can build ProductData safely.
    ...
    for i, (scene, image) in enumerate(zip(scenes, images), start=1):
        ...
        script_scenes.append({
            ...
            "sfx": scene.sfx,
            "notes": "auto-generated editorial scene; review before publishing",
            "transition": "cut"
        })
    ...
    (project.product_dir / "script.json").write_text(
